...
void btn_draw_textmullinebtn(_btn_obj * btnx)
{
    ...
    u8 len;
    u8 btn_show_str[10];
//    u8 str_index = 0,line_len = 0;
	sta=btnx->sta&0x03;	  
    btnx->bcfucolor = 0x1210;//字体颜色
    btnx->font = 12;        //子图大小
	switch(sta)
	{
		case BTN_RELEASE://正常(松开)		
			...
			*/
            if(gcodefile_t[gcodefile_index_base+gcodefile_index_sub].file_type == GCODE) {
                //gui_show_strmid(btnx->left+btnx->caption_left,btnx->top+btnx->caption_top+16,btnx->width-btnx->caption_left,btnx->height-btnx->caption_top,btnx->bcfucolor,btnx->font,".GCODE",1);  
            }
            len = strlen(btnx->caption);
            if(len > 11){
                //判断第9位是否为中文字符
                #ifdef SHOW_BUG
                if(btn_show_str[str_index+7] >= 0x80)   line_len = 7;//中文
                else                                    line_len = 8;//非中文
                btn_show_str[7] = '\0'; 
                str_index += line_len;
                strncpy(btn_show_str,btnx->caption+str_index,line_len);
                #else
                ...
                strncpy(btn_show_str,btnx->caption+str_index,line_len);
                #else
                strncpy(btn_show_str,btnx->caption+11,11);
                #endif
