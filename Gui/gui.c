...
//20150909
/***********************************
	功能：读取Flash中的图标数据进行显示    最大356（u8）
	参数1：x，       图形起始点的x坐标
	参数2：y，       图形起始点的y坐标
	参数3：width      图形宽
	参数4：height      图形高
  参数5：address_offset   图标在外部Flash中的偏离ICON_START_ADDR的偏移地址
	参数6：pen_color，图形颜色
		...
		for(j=0;j<height;j++)	//列循环
		{
			for(i=0;i<width;i++)//写一行像素
			 {  	
				 ...
				 gui_phy.draw_point(x+i,y+j,color);//显示某点
			 }
			 sum = sum+group+1;
		}
	}
...
//从sd卡读取方式
	...
	if(filp==NULL) return;
	if(f_open(filp,index,FA_READ))
	{
		myfree(SRAMIN,filp);
		return;
	}
	if(f_read(filp,iconbuf,2,&br))
	{
		myfree(SRAMIN,filp);
		return;
	}
	//第一个字节为宽度  第二个字节为高度
	if((width==iconbuf[0]) && (height==iconbuf[1]))
	{
		for(j=0;j<height;j++)	//列循环
		{
			for(i=0;i<width;i++)//写一行像素
			 {  	
				 ...
				 if(offset==0)
				 {
				 	//icon_buf = iconbuf[sum+group+2];	//从第三个字节开始
					f_lseek(filp,sum+group+2);
					f_read(filp,&icon_buf,1,&br);
				 }
				 if(icon_buf & (0x80>>offset))
				 {	
					 color=pen_color;//
				 }		    
			 	 else
