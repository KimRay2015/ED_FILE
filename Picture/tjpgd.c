...
u8 jpeg_mallocall(void)
{
	...
	jpeg_dev=(JDEC*)mymalloc(SRAMIN,sizeof(JDEC));
	if(jpeg_dev==NULL)return PIC_MEM_ERR;		//申请内存失败.
	jpg_buffer=(u8*)mymalloc(SRAMIN,JPEG_WBUF_SIZE);
...
void jpeg_freeall(void)
{
	...
	myfree(SRAMIN,jpeg_dev);		//释放jpeg_dev申请到的内存
...
{ 
	...
	u32 filllenth=(u32)width*height;		//获取此次填充长度
   	pic_phy.set_window(rect->left+picinfo.S_XOFF,rect->top+picinfo.S_YOFF,width,height);//设置窗口大小
	pic_phy.fill_prepare();	//开始写入GRAM 
	while(filllenth--) 		//控制显示区域的高度
	{    
		pic_phy.fill_point(*pencolor); 
		pencolor++;		
	} 	 
	...
	if(res==0)
	{
		...
		{ 
			...
			{ 	
				...
				if(win)pic_phy.set_window(0,0,picinfo.lcdwidth,picinfo.lcdheight);//将窗口恢复原样
			}
		} 
