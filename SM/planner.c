...
float max_feedrate[4]=DEFAULT_MAX_FEEDRATE;					//可允许的最大速度
float axis_steps_per_unit[4]=DEFAULT_AXIS_STEPS_PER_UNIT;	//移动1mm的step数
unsigned long max_acceleration_units_per_sq_second[4]=DEFAULT_MAX_ACCELERATION;	//最大加速度
...
#ifdef COREXY
  if((DISABLE_X) && (x_active == 0) && (DISABLE_Y) && (y_active == 0))
  {
		disable_x();
		disable_y();
  }
#else			//Modified by Kim 20191122
...
//	printf("FAN_PIN:%d ",tail_fan_speed);
#endif//FAN_PIN > -1
#ifdef AUTOTEMP
  getHighESpeed();
#endif
...
void plan_buffer_line(const float x, const float y, const float z, const float e, float feed_rate, const uint8_t extruder)
{
  ...
  if(mbl_active)
  {
	target[Z_AXIS] = round((z+mbl_get_z(x, y))*axis_steps_per_unit[Z_AXIS]);
  }
  else
...
// Number of steps for each axis
  ...
  block->steps_e /= 100;
 #if 0
 	if(abs(block->steps_e)>500)	//e轴单次行走长度为10
 	{
		printf("E:%ld\r\n",block->steps_e);
  ...
  block->step_event_count = max(block->steps_x, max(block->steps_y, max(block->steps_z, block->steps_e)));
	...
	USR_DbgLog("\r\nX:%ld",block->steps_x);
	USR_DbgLog("Y:%ld",block->steps_y);
	USR_DbgLog("Z:%ld",block->steps_z);
	USR_DbgLog("E:%ld",block->steps_e);
	USR_DbgLog("block->step_event_count:%ld",block->step_event_count);
  ...
  block->direction_bits = 0;
	...
	//USR_DbgLog("block->direction_bits:%ld",block->direction_bits);
  ...
  //block->nominal_rate = block->nominal_rate/2;
...
#define MAX_FREQ_TIME (1000000.0/XY_FREQUENCY_LIMIT)
  ...
  block->acceleration = block->acceleration_st / steps_per_mm;
	...
	/*
	block->acceleration_st *x x根据定时器的频率进行计算;
	X:越大->
	X:越小->
	*/
	#ifdef __TIM2_CCRV_MOTO
		 block->acceleration_rate = (long)((float)block->acceleration_st * 8.194304);//(16777216.0/(16000000/8)));
	#else
		block->acceleration_rate = (long)((float)block->acceleration_st * 8.388608);
	#endif
  //block->acceleration_rate = (long)((float)block->acceleration_st * 4.194304);		//(16777216.0/(F_CPU/8)))

	//if(feedmultiply > 200)				//Acceleration limit add by kim 20191126
	//
//		block->acceleration = block->acceleration;
//		block->acceleration_rate = block->acceleration_rate;
	//}
...
void plan_set_position(const float x, const float y, const float z, const float e)
{
  ...
  if(mbl_active)
  {
	position[Z_AXIS] = round((z+mbl_get_z(x, y))*axis_steps_per_unit[Z_AXIS]);
  }
  else
...
{
	if((block_buffer_tail+BLOCK_BUFFER_SIZE-1)%BLOCK_BUFFER_SIZE == block_buffer_head)
	{
		return TRUE;
	}
	else
