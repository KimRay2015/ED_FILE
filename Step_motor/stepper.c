...
unsigned short calc_timer(unsigned short step_rate)
{
	...
	timer = 2000000 / step_rate - 1;			//default 2000000
	if(timer < 150) 
	{
...
void App_MotoRunning(void)
{
	...
	if(current_block == NULL) 
	{
		...
		if(current_block != NULL) 
		{
			// 	if(PrintInfo.printsd==2)	//打印过程中暂定时不处理任何事
			// 	{
			//		return;
			//	}
			...
			if(step_events_completed >= current_block->step_event_count) 
			{
				...
				if(BoardCheckFlg == 1) 
				{
					if(blocks_queued() == FALSE && buflen == 0) 
					{
						BoardCheckFlg = 2;
					}
				}
