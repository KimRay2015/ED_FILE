...
{  
    ...
    for(i=0;i<8;i++)                         // 循环8次
    {
        if(num&0x80) TDIN=1;                             //若最到位为高，则输出高
        else         TDIN=0;                             //若最到位为低，则输出低
        num <<= 1;                           			  // 低一位移位到最高位
        TCLK=0;			//上升沿有效
		delay_us(1);
		TCLK=1; 
...
{ 	 
	...
	delay_us(6);							//转换时间最长为6us
	TCLK=0;
	delay_us(1);
	TCLK = 1;								//给一个时钟，清除BUSY
	delay_us(1);
	TCLK=0; 
    for(i=0;i<16;i++)                       // 读16位数据
    {
		Temp <<=1;		
        TCLK=0;			//下降沿有效
		delay_us(1);
...
//	temp=0XBB;//标记校准过了
	AT24CXX_WriteOneByte(SAVE_ADDR_BASE+13,0xBB);
	temp=AT24CXX_ReadOneByte(SAVE_ADDR_BASE+13);
//	AT24CXX_Read(0,datatemp,25);
...
{								 
	...
	LCD_Clear(WHITE);//清屏 	   
	POINT_COLOR=BLACK;
	LCD_ShowString(40,40,240,160,16,(u8*)TP_REMIND_MSG_TBL);//显示提示信息
...
{	
	...
	{ 										    
		LCD_Clear(WHITE);//清屏
		GPIO_SetBits(GPIOD,GPIO_Pin_2);		//校正屏前打开背光
		TP_Adjust();  //屏幕校准 
		//TP_Save_Adjdata();	 
	}			
	TP_Get_Adjdata();	
